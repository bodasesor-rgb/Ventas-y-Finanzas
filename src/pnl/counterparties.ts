...
const PARTNER_KEYS = PARTNER_NAMES.map((n) => ({
  name: n,
 ...
 */
export function extractBeneficiary(description: string): string | null {
...
export function looksLikeOutboundTransfer(description: string): boolean {
  const d = String(description || "");
  return (
    /\bPAGO INTERBANCARIO A\b/i.test(d) ||
...
export function classifyCounterparty(
  description: string
): CounterpartyHit | null {
  if (!looksLikeOutboundTransfer(description)) return null;
...
export function applyCounterpartyCategories(lines: BankLine[]): BankLine[] {
  return lines.map((line) => {
    if (line.direction === "abono") return line;
