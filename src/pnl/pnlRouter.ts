...
pnlRouter.get("/api/pnl/library", async (_req, res) => {
  let restore:
    | { restored: string[]; skipped: string[]; errors: string[] }
    | undefined;
  try {
    if (shouldAutoRestoreOnce()) {
      restore = await restoreArchivesFromDrive();
...
pnlRouter.post("/api/pnl/restore-from-drive", async (_req, res) => {
  try {
    const result = await restoreArchivesFromDrive();
    res.json({
      ok: true,
      ...result,
...
pnlRouter.get("/api/pnl/runs/:id/pdf", async (req, res) => {
  const run = loadRuns().find((r) => r.id === req.params.id);
  if (!run) {
  ...
  try {
    const pdf = await resolvePdfBytes(run);
    if (!pdf) {
      res.status(404).json({
        ok: false,
...
pnlRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      let archive:
        | { ok: true; pdfUrl?: string; version?: string }
        | { ok: false; error: string } = {
        ok: false,
        error: "no intentado",
      };
      try {
        const archived = await archiveStatementToDrive(
          run,
          saved.storedPath
        );
...
pnlRouter.get("/api/pnl/apps-script-status", async (_req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      message: needsPublish
        ...
        : hasErSheet
          ? `Apps Script ${version}: listo · Sheet «${
              data.spreadsheetName || "?"
            }» tiene Estado de Resultados.`
          : `Apps Script ${version}: script OK, pero la pestaña aún no existe en «${
              data.spreadsheetName || "el Sheet"
            }». Pulsa Crear pestaña.`,
    });
  } catch (err) {
    res.status(502).json({
...
pnlRouter.post("/api/pnl/setup-estado-resultados", async (_req, res) => {
  try {
    ...
    if (!result.ok) {
      res.status(502).json({
        ...
        hint:
          "Pega Codigo.gs v18 en Apps Script → Guardar → Implementar → Nueva versión. Luego reintenta.",
      });
      return;
    }
    res.json({
    ...
    const looksOld =
      /values no es array|v16|v19|action/i.test(error) ||
    ...
    res.status(502).json({
      ...
      error: looksOld
        ...
        : error,
      hint:
        "Pega Codigo.gs v20 → Guardar → Implementar → Nueva versión (misma URL /exec). Luego vuelve a pulsar Crear pestaña.",
    });
  }
});
...
pnlRouter.post("/api/pnl/runs/:id/send-to-sheet", async (req, res) => {
  ...
  const run = runs[idx];
  try {
    const result = await sendRunToBancoSheet(run);
    ...
    res.status(502).json({
      ...
      error,
      hint:
        "Pega Codigo.gs v19 en Apps Script, ejecuta restoreEstadoResultados_ y publica Nueva versión. El Sheet debe mostrar Estado de Resultados (Banco queda oculto).",
    });
  }
});
