...
pnlRouter.get("/api/pnl/apps-script-status", async (_req, res) => {
  ...
  try {
    ...
    const hasErSheet = sheets.some((s) =>
      String(s).startsWith("Estado de Resultados")
    );
    const needsPublish =
      !hasErFlag && !/v1[89]|v2\d/.test(version) && !hasErSheet;
    res.json({
      ok: true,
      version,
...
pnlRouter.post("/api/pnl/setup-estado-resultados", async (_req, res) => {
  try {
    ...
    const looksOld =
      /values no es array|v16|action/i.test(error) ||
    ...
    res.status(502).json({
      ...
      error: looksOld
        ...
        : error,
      hint:
        "Pega Codigo.gs v18 → Guardar → Implementar → Nueva versión (misma URL /exec). Luego vuelve a pulsar Crear pestaña.",
    });
  }
});
