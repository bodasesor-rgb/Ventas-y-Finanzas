import { classifyCounterparty, PARTNER_NAMES } from "./counterparties";
import type { BankLine, StatementRun } from "./types";
...
export interface MonthAnalysis {
  ...
  servicios: number;
  otrosGastos: number;
  topProveedores: { name: string; total: number; payments: number }[];
  cuadra: boolean | null;
}
...
export interface YearAnalysis {
  ...
  byCategory: { id: string; total: number }[];
}
...
function absGasto(n: number): number {
  return Math.abs(Math.min(0, n));
}

function lineCounterparty(line: BankLine): {
  name: string;
  kind: "socio" | "proveedor";
} | null {
...
export function buildYearAnalysis(
  runs: StatementRun[],
  year = 2026
): YearAnalysis {
  ...
  for (const run of yearRuns) {
    ...
    const monthProv = new Map<string, { total: number; payments: number }>();
    let socios = 0;
    let proveedores = 0;
    let otrosGastos = 0;

    for (const line of run.lines || []) {
      if (line.direction === "abono" || line.amount >= 0) continue;
      ...
      if (cp) {
        ...
        !["ads", "apps", "pass", "comisiones", "servicios"].includes(
          line.category
        )
      ) {
        otrosGastos += spend;
  ...
  const providers = Array.from(byProvider.values()).filter(
    (p) => p.kind === "proveedor"
  );
  const socios = Array.from(byProvider.values()).filter(
    (p) => p.kind === "socio"
  );
  const proveedoresTotal = providers.reduce((s, p) => s + p.total, 0);
  const sociosTotal = socios.reduce((s, p) => s + p.total, 0);

  const topProveedores: ProviderRankRow[] = providers
    .map((p) => ({
      name: p.name,
      kind: p.kind as "proveedor",
      ...
      byMonth: Object.fromEntries(
        Object.entries(p.byMonth).map(([k, v]) => [k, round2(v)])
      ),
    }))
    .sort((a, b) => b.total - a.total);
