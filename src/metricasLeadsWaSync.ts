...
export function classifyStatusName_(
  name: string
 ...
 */
export function countsAsCotizacionMail_(opts: {
  subject: string;
  type?: string;
}): boolean {
  const n = normLabel_(opts.subject);
  if (!n) return false;
  if (n.includes("lucy") || n.includes("agente virtual")) return false;
  if (
  ...
  if (!isOutgoingMailEvent_({ type: opts.type, subject: opts.subject })) {
    // mail_message_note sí; cotizacion_note/chats no
    const t = String(opts.type || "");
    if (t !== "mail_message_note" && t !== "mail_message") return false;
  }
...
async function loadLayout_(): Promise<Layout> {
  ...
  for (let r = sectionStart + 1; r < sectionEnd; r++) {
    const lab = normLabel_(values[r]?.[labelCol]);
    if (lab === "leads" && rows.leads < 0) rows.leads = r;
    else if (lab === "correo" && rows.correo < 0) rows.correo = r;
    else if (lab.includes("no contestaron") && rows.noContestaron < 0)
      rows.noContestaron = r;
    else if (lab === "llenado" && rows.llenado < 0) rows.llenado = r;
  ...
  return {
    ...
    rows: {
      ...
      noContestaron:
        rows.noContestaron >= 0 ? rows.noContestaron + 1 : -1,
...
export type WeekLeadsWa = {
  ...
  noContestaron: number;
  llenado: number;
  porcentaje: number; // 0–1 fraction for sheet % format
};

function buildStatusMaps_(pipeline: KommoPipeline): {
...
export async function syncMetricasLeadsWa(opts?: {
  ...
  statusMap?: Array<{ id: number; name: string; class: string }>;
  updatedCells: number;
  weeks: WeekLeadsWa[];
  skipped?: boolean;
  error?: string;
  ...
  if (!maps.noContestaronIds.size && !maps.llenadoIds.size) {
    return {
      ...
      weeks: [],
      error:
        'No encontré etapas "Datos de clientes" / "Humano trabaja" / "Seguimientos" / "Intención de paga" en el pipeline',
  ...
  let rangeToMs =
    targetWeeks[targetWeeks.length - 1].date.getTime() + 7 * 86400000 - 1000;
  ...
  for (const w of targetWeeks) {
    ...
    if (weekEndMs > Date.now() + 1000) weekEndMs = Date.now() + 1000;
  ...
  const sheets = google.sheets({ version: "v4", auth: auth as never });
  const data: Array<{ range: string; values: (number | string)[][] }> = [];
