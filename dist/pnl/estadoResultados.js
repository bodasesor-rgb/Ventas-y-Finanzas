...
exports.buildEstadoResultados = buildEstadoResultados;
const MONTH_LABELS = [
    "enero",
    "febrero",
    "marzo",
...
function sumArr(arr) {
    return round2(arr.reduce((s, v) => s + (v == null ? 0 : v), 0));
}
function hasAny(arr) {
...
function line(id, label, pick, buckets, tone) {
...
function section(id, label, tone) {
    return {
        id,
        label,
        kind: "section",
...
function totalRow(id, label, sources, tone) {
    const months = empty12().map((_, mi) => {
        const any = sources.some((r) => r.months[mi] != null);
        if (!any)
            return null;
        return round2(sources.reduce((s, r) => s + (r.months[mi] == null ? 0 : Number(r.months[mi])), 0));
    });
    return {
        id,
        label,
...
function diffRow(id, label, a, b, tone) {
    const months = empty12().map((_, mi) => {
        if (a.months[mi] == null && b.months[mi] == null)
            return null;
...
function marginRow(id, label, num, den) {
    const months = empty12().map((_, mi) => {
        const d = Number(den.months[mi] || 0);
        const n = Number(num.months[mi] || 0);
        if (den.months[mi] == null && num.months[mi] == null)
            return null;
        if (d === 0)
            return null;
        return round2(n / d);
...
function buildEstadoResultados(runs, year = 2026) {
    ...
    const cateringI = line("catering_i", "Catering", (b) => (b.filled ? 0 : 0), buckets, "muted");
    ...
    const totIngreso = totalRow("tot_ingreso", "TOTAL", [intereses, venta, ingreso, cateringI, mobiliarioI, lugaresI, showsI], "income");
    ...
    const totEgreso = totalRow("tot_egreso", "TOTAL", [proveedor, evento, banquete, cateringE, mobiliarioE, lugaresE, showsE], "expense");
    rows.push(totEgreso);
    ...
    const programas = line("programas", "Programas", (b) => round2(b.apps + b.pass), buckets, "expense");
    ...
    const otrosG = line("otros_g", "Otros", (b) => round2(b.comisiones +
        b.servicios +
        b.transferencia +
        b.revisar +
        b.otro +
        b.otros), buckets, "expense");
    rows.push(marketing, rh, programas, impuestos, otrosG);
    const totGastos = totalRow("tot_gastos", "TOTAL", [marketing, rh, programas, impuestos, otrosG], "expense");
    rows.push(totGastos);
...
//# sourceMappingURL=estadoResultados.js.map