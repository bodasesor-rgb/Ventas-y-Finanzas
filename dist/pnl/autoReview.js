...
function scoreReconciliation(rec) {
    ...
    if (rec.diffIngresos != null)
        s -= Math.min(50, Math.abs(rec.diffIngresos) / 1000);
    if (rec.diffGastos != null)
        s -= Math.min(50, Math.abs(rec.diffGastos) / 1000);
    return s;
}
function money(n) {
...
function totalsWithout(lines, dropId) {
    return (0, parseStatement_1.summarizeTotals)(lines.filter((l) => l.id !== dropId));
}
/** Invierte el signo de una línea. */
function totalsWithFlip(lines, flipId) {
    return (0, parseStatement_1.summarizeTotals)(lines.map((l) => l.id === flipId
...
function findSuspects(lines, text, rec) {
    ...
    for (const line of lines) {
        ...
        if (diffIng != null && !rec.matchIngresos && near(abs, Math.abs(diffIng), 0.05)) {
            push({
                ...
                suggestedAmount: line.amount > 0 && diffIng > 0
                    ? 0
                    : line.amount > 0
    ...
    for (const line of lines) {
        ...
        if (r.matchCompleto && !rec.matchCompleto) {
            ...
            const betterIng = diffIng != null &&
                r.diffIngresos != null &&
                Math.abs(r.diffIngresos) < Math.abs(diffIng) - TOL;
            const betterGas = diffGas != null &&
                r.diffGastos != null &&
            ...
            if (betterIng || betterGas) {
                push({
                    ...
                    description: line.description,
                    amount: line.amount,
                    reason: `Excluir este movimiento reduce el descuadre (diff dep ${r.diffIngresos ?? "—"}, cargos ${r.diffGastos ?? "—"})`,
                });
            }
        }
    }
    ...
    if (!rec.matchIngresos && diffIng != null && Math.abs(diffIng) > TOL) {
        for (const line of lines) {
            if (line.amount <= 0)
                continue;
            const suggested = Math.round((line.amount - diffIng) * 100) / 100;
            if (suggested < 0 || near(suggested, line.amount, 0.005))
                continue;
            ...
            if (r.matchIngresos) {
                push({
                    ...
                    description: line.description,
                    amount: line.amount,
                    reason: `Ajustar ingreso a ${money(suggested)} cierra la diferencia de depósitos`,
    ...
    if (!rec.matchGastos && diffGas != null && Math.abs(diffGas) > TOL) {
        for (const line of lines) {
            if (line.amount >= 0)
                continue;
            const suggested = Math.round((line.amount - diffGas) * 100) / 100;
            if (suggested > 0 || near(suggested, line.amount, 0.005))
                continue;
            const t = totalsWithAmount(lines, line.id, suggested);
            const r = (0, statementSummary_1.reconcileTotals)(oficial, t, TOL);
            if (r.matchGastos) {
                push({
                    ...
                    description: line.description,
                    amount: line.amount,
                    reason: `Ajustar cargo a ${money(suggested)} cierra la diferencia de otros cargos`,
                    suggestedAmount: suggested,
                });
            }
        }
                ...
                if (near(sum, Math.abs(diffIng), 0.05)) {
                    for (const line of [incomes[i], incomes[j]]) {
                        push({
                            ...
                            description: line.description,
                            amount: line.amount,
                            reason: `Par de ingresos suma ${money(sum)} ≈ diff depósitos; revisa ambos`,
                        });
                    }
                    break outerIng;
                }
                ...
                if (near(sum, Math.abs(diffGas), 0.05)) {
                    for (const line of [expenses[i], expenses[j]]) {
                        push({
                            ...
                            description: line.description,
                            amount: line.amount,
                            reason: `Par de cargos suma ${money(-sum)} ≈ diff otros cargos; revisa ambos`,
                        });
                    }
                    break outerGas;
                }
...
function suspectPriority(s) {
    ...
    if (r.includes("signo parece invertido"))
        return 95;
    if (r.includes("≈ diferencia"))
        return 90;
    if (r.includes("discrepancia") || r.includes("impreso"))
        return 70;
    if (r.includes("par de"))
        return 60;
    if (r.includes("reduce el descuadre"))
        return 40;
    if (r.includes("ajustar"))
        return 20;
    return 10;
}
function rankAndDedupeSuspects(found) {
    const sorted = found
        .slice()
    ...
    const smoking = unique.find((s) => suspectPriority(s) >= 90 && /excluye|invertido|≈ diferencia/i.test(s.reason));
...
function runAutoReview(text, rules) {
    ...
    let message;
    if (matched) {
        message = `Cuadra tras ${passes.length} lectura(s). Mejor estrategia: ${STRATEGIES.find((s) => s.id === best.strategy)?.label || best.strategy}.`;
    }
    else if (suspects.length) {
        const top = suspects[0];
        message = `Tras ${passes.length} lecturas aún no cuadra. Cuenta sospechosa: ${top.date || "?"} · ${top.description.slice(0, 60)} · ${money(top.amount)}. ${top.reason}. Se marcaron ${suspects.length} movimiento(s) en Revisar.`;
    }
    else {
        message = `Tras ${passes.length} lecturas no se encontró una sola cuenta culpable. Diff depósitos: ${reconciliation.diffIngresos ?? "—"}; diff cargos: ${reconciliation.diffGastos ?? "—"}. Revisa montos a mano.`;
    }
    const autoReview = {
        ranAt: new Date().toISOString(),
        matched,
