...
exports.pruneMerchantCategories = pruneMerchantCategories;
...
function ensureBroadCategory(id, label, kind) {
...
function guessBroadCategory(description, merchant) {
    const desc = description.toLowerCase();
    const token = (merchant || "").split(/\s+/)[0].toLowerCase();
    if (MERCHANT_CATEGORY[token])
        return MERCHANT_CATEGORY[token];
    for (const [k, cat] of Object.entries(MERCHANT_CATEGORY)) {
        if (desc.includes(k))
            return cat;
    }
    // Heurística apps / software
    if (/\b(dublin|san francisco|digital goods|ai powered|software|saas|app)\b/i.test(description)) {
        return "apps";
    }
    if (/\b(facebk|facebook|meta|google|ads)\b/i.test(desc))
        return "ads";
    ...
    if (/\bpago\b/i.test(desc))
        return "pago";
    return "revisar";
}
function remapNarrowCategory(categoryId) {
    if (BROAD_CATS.has(categoryId))
        return categoryId;
    // Categorías viejas auto-creadas tipo "netlify_san" → apps
    const base = categoryId.split("_")[0].toLowerCase();
    if (MERCHANT_CATEGORY[base])
        return MERCHANT_CATEGORY[base];
    if (/netlify|openai|cursor|replit|ebanx|shopify|gamma|anthropic|sendin|brevo|vercel|github|figma|notion/i.test(categoryId)) {
        return "apps";
    }
    if (/facebk|facebook|meta|google|payu/i.test(categoryId))
        return "ads";
    return "revisar";
}
/**
...
function autoCreateCategoriesFromLines(lines) {
    ...
    const created = []; // ya no creamos categorías merchant
    const rulesCreated = [];
    const out = lines.map((line) => {
        ...
        if (merchant) {
            ...
            const genericMatch = /^(instrucciones|interbancario|terceros|penalizacion|aut|para|suc|ref|mismo|dia|transferencia)$/i;
            if (matchToken.length >= 4 &&
                ...
                if (!rulesCreated.includes(label))
                    rulesCreated.push(label);
                return {
                    ...line,
                    category: broad,
...
function pruneMerchantCategories() {
    const cats = (0, store_1.loadCategories)();
    const removed = [];
    const cleaned = cats.filter((c) => {
        if (BROAD_CATS.has(c.id) || c.builtin)
            return true;
        if (c.autoCreated) {
            removed.push(c.id);
            return false;
