...
exports.detectPeriodFromText = detectPeriodFromText;
const MONTHS_ES = {
    enero: "01",
    febrero: "02",
    marzo: "03",
...
function detectPeriodFromText(text) {
    ...
    let year = null;
    let month = null;
    const periodo = t.match(/(?:Per[ií]odo|Periodo|Fecha de corte)[^0-9]{0,40}?(\d{1,2})\s+de\s+([A-Za-zÁÉÍÓÚáéíóú]+)\s+(?:del?\s+)?(\d{4})/i);
    if (periodo) {
    ...
    if (!year || !month) {
        const any = t.match(/\b(\d{1,2})\s+de\s+([A-Za-zÁÉÍÓÚáéíóú]+)\s+(?:del?\s+)?(\d{4})\b/i);
        if (any) {
    ...
    if (!year || !month) {
        ...
        const glued = t.match(/\b(\d{1,2})(ENE|FEB|MAR|ABR|MAY|JUN|JUL|AGO|SEP|OCT|NOV|DIC)(20\d{2})\b/i);
        if (glued) {
            ...
            const dm = t.match(/\b(\d{1,2})(ENE|FEB|MAR|ABR|MAY|JUN|JUL|AGO|SEP|OCT|NOV|DIC)\b/i);
            if (y && dm) {
                year = Number(y[1]);
