...
exports.syncMetricasVisitas = syncMetricasVisitas;
exports.metricasVisitasStatus = metricasVisitasStatus;
...
function yyyymmddToUtc_(yyyymmdd) {
...
async function loadMetricasViaCsv_() {
    const id = (0, googleAuth_1.metricasSheetId)();
    const sheetName = (0, googleAuth_1.metricasSheetName)();
    const url = `https://docs.google.com/spreadsheets/d/${id}/gviz/tq?tqx=out:csv&sheet=${encodeURIComponent(sheetName)}`;
    const res = await fetch(url, { redirect: "follow" });
    if (!res.ok)
        throw new Error(`CSV Metricas HTTP ${res.status}`);
...
function detectLayout_(sheetName, values) {
...
function buildWeekPayload_(layout, daily, opts) {
    ...
    for (const w of layout.weekCols) {
        ...
        const filterDaily = (rows) => rows.filter((r) => {
        ...
        const organic = filterDaily(daily.organic).reduce((a, b) => a + b.sessions, 0);
        const blogs = filterDaily(daily.blogs).reduce((a, b) => a + b.sessions, 0);
        const colecciones = filterDaily(daily.colecciones).reduce((a, b) => a + b.sessions, 0);
        ...
        out.push({
            ...
            weekStartIso: `${w.date.getUTCFullYear()}-${pad2_(w.date.getUTCMonth() + 1)}-${pad2_(w.date.getUTCDate())}`,
            col: w.col,
            site,
            organic,
            blogs,
...
async function writeViaSheetsApi_(layout, weeks) {
    const auth = await (0, googleAuth_1.getGoogleAuthClient)([
        "https://www.googleapis.com/auth/spreadsheets",
    ]);
    const sheets = googleapis_1.google.sheets({ version: "v4", auth: auth });
...
async function writeViaAppsScript_(weeks) {
    ...
    return {
        updatedCells: Number(result.updatedCells || weeks.length),
        method: "apps_script",
 ...
 */
async function syncMetricasVisitas(opts) {
    const cfg = (0, ga4Client_1.ga4Configured)();
    if (!cfg.ok) {
        return {
            ok: false,
            sheetName: (0, googleAuth_1.metricasSheetName)(),
            ...
            hint: "En Hostinger agrega GA4_PROPERTY_ID y GOOGLE_SERVICE_ACCOUNT_JSON. Comparte el Sheet con el email del service account (Editor) y dale Viewer en la propiedad GA4.",
        };
    }
    const lookbackDays = opts?.lookbackDays ?? 120;
    const end = new Date();
    const start = new Date(end.getTime() - lookbackDays * 86400000);
    const startDate = `${start.getUTCFullYear()}-${pad2_(start.getUTCMonth() + 1)}-${pad2_(start.getUTCDate())}`;
    const endDate = `${end.getUTCFullYear()}-${pad2_(end.getUTCMonth() + 1)}-${pad2_(end.getUTCDate())}`;
    let layout;
    try {
        layout = await loadMetricasViaSheetsApi_();
    }
    catch (err) {
        console.warn("[metricas-visitas] Sheets API falló, intento CSV", err instanceof Error ? err.message : err);
        layout = await loadMetricasViaCsv_();
    }
    const daily = await (0, ga4Client_1.fetchGa4VisitasDaily)({ startDate, endDate });
    ...
    try {
        ...
        console.warn("[metricas-visitas] write Sheets API falló, intento Apps Script", err instanceof Error ? err.message : err);
        try {
