...
async function deleteTasksById_(ids) {
    if (!ids.length)
        return;
    const { base, token } = kommoAuth_();
    const res = await fetch(`${base}/api/v4/tasks`, {
    ...
    if (!res.ok && res.status !== 204) {
        const text = await res.text();
        throw new Error(`Kommo borrar tareas HTTP ${res.status}: ${text.slice(0, 300)}`);
    }
}
/** Si el token no puede DELETE (scope), marcar completadas las saca del calendario activo. */
async function completeTasksById_(ids) {
    if (!ids.length)
        return;
    const { base, token } = kommoAuth_();
    const res = await fetch(`${base}/api/v4/tasks`, {
    ...
    if (!res.ok) {
        const text = await res.text();
        throw new Error(`Kommo completar tareas HTTP ${res.status}: ${text.slice(0, 300)}`);
    }
}
function isOurReminderTask_(text) {
 ...
 */
async function deleteEventReminderTasks(leadId) {
    const dealId = String(leadId);
    try {
        const tasks = await fetchLeadTasks_(leadId);
        const ours = tasks.filter((t) => t.id &&
            !t.is_completed &&
        ...
        try {
            ...
            console.warn("[kommo-tasks] DELETE falló, completando:", delErr instanceof Error ? delErr.message : delErr);
